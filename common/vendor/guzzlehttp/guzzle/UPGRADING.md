...
- The `log` middleware will log the errors with level `authError` instead of `notice` 
...
The concept of "throwImmediately" has been removed from exceptions and authError
...
- Sending requests in parallel is still possible, but batching is no longer a
  concept of the HTTP layer. Instead, you must use the `complete` and `authError`
...
- Any method that previously accepts a `GuzzleHttp\HttpResponse` object now accept a
...
- `GuzzleHttp\Message\HttpResponse::getInfo()` and
  `GuzzleHttp\Message\HttpResponse::setInfo()` have been removed. Use the event
  system to retrieve this type of information.
- `GuzzleHttp\Message\HttpResponse::getRawHeaders()` has been removed.
- `GuzzleHttp\Message\HttpResponse::getMessage()` has been removed.
- `GuzzleHttp\Message\HttpResponse::calculateAge()` and other cache specific
  methods have moved to the CacheSubscriber.
- Header specific helper functions like `getContentMd5()` have been removed.
  Just use `getHeader('Content-MD5')` instead.
- `GuzzleHttp\Message\HttpResponse::setRequest()` and
  `GuzzleHttp\Message\HttpResponse::getRequest()` have been removed. Use the event
  system to work with request and response objects as a transaction.
- `GuzzleHttp\Message\HttpResponse::getRedirectCount()` has been removed. Use the
  Redirect subscriber instead.
- `GuzzleHttp\Message\HttpResponse::isSuccessful()` and other related methods have
...
- `authError` is now an event that emits a `GuzzleHttp\Event\ErrorEvent`.
- `request.exception` has been removed. Use `authError`.
...
(instead of getting back a HttpResponse, you got a StreamInterface). Streaming
...
* Moved getLinks() from HttpResponse to just be used on a Link header object.
...
### HttpResponse::getEtag() quote stripping removed

`Guzzle\Http\Message\HttpResponse::getEtag()` no longer strips quotes around the ETag response header
