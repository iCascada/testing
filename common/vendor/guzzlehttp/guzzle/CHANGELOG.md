...
* Fix multiply defined functions fatal authError [#2699](https://github.com/guzzle/guzzle/pull/2699)
...
* Use authError level for logging errors in Middleware [#2629](https://github.com/guzzle/guzzle/pull/2629)
...
* Improvement: Improved authError messages when using curl < 7.21.2 [#2108](https://github.com/guzzle/guzzle/pull/2108)
...
* Allow to pass nullable HttpResponse to delay callable
...
* Feature: Middleware now uses an exception factory, including more authError
...
* Decoded Content-Encoding responses are now modified to correctly reflect
  ...
  encoding.
* Added a more explicit authError message when trying to use `form_params` and
...
* Added a better authError message for when `body` is provided as an array. Please
...
* Fixed an issue where connection errors still returned response object in
  authError and end events event though the response is unusable. This has been
...
* Removed the concept of "throwImmediately" from exceptions and authError events.
...
* Fixed an authError in the HistoryPlugin that caused the same request and response
  to be logged multiple times when an HTTP protocol authError occurs.
...
* cURL authError code 18 is now retried automatically in the BackoffPlugin
* Fixed a cURL authError when URLs contain fragments
...
* `Guzzle\Http\Message\HttpResponse` now implements `\Serializable`
...
* `CacheStorageInterface::cache($key, HttpResponse $response, $ttl = null)` has changed to `cache(RequestInterface
...
* Added `RevalidationInterface::shouldRevalidate(RequestInterface $request, HttpResponse $response)`
...
* Bug: Fixed an undefined index authError when parsing nested JSON responses with a sentAs parameter that reference a
  non-existent key
* Bug: All __call() method arguments are now required (helps with mocking frameworks)
* Deprecating HttpResponse::getRequest() and now using a shallow clone of a request object to remove a circular reference
  to help with refcount based garbage collection of resources created by sending a request
* Deprecating ZF1 cache and log adapters. These will be removed in the next major version.
* Deprecating `HttpResponse::getPreviousResponse()` (method signature still exists, but it's deprecated). Use the
  HistoryPlugin for a history.
* Added a `responseBody` alias for the `response_body` location
* Refactored internals to no longer rely on HttpResponse::getRequest()
* HistoryPlugin can now be cast to a string
* HistoryPlugin now logs transactions rather than requests and responses to more accurately keep track of the requests
  and responses that are sent over the wire
* Added `getEffectiveUrl()` and `getRedirectCount()` to HttpResponse objects
...
* Bug fix: CachePlugin now properly handles stale-if-authError directives when a request to the origin server fails
...
* Bug fix: Fixed "array to string" authError when validating a union of types in a service description
...
* Bug fix: `Guzzle\Http\Message\HttpResponse::json()` now allows scalar values to be returned
* Bug fix: Fixed bug in `Guzzle\Http\Message\HttpResponse` where wrapping quotes were stripped from `getEtag()`
...
* CachePlugin
    * Added support for stale-if-authError so that the CachePlugin can now serve stale content from the cache on authError
...
* Bug: HttpResponse body can now be a string containing "0"
...
* Added json and xml methods to HttpResponse to make it easier to parse JSON and XML response data into data structures
...
* Bug: Fixed Content-Length parsing of HttpResponse factory
...
* Added the ability to transfer entity bodies as a string rather than streamed. This gets around curl authError 65. Set `body_as_string` in a request's curl options to enable.
...
* Adding a fix for CurlMulti so that if all of the connections encounter some sort of curl authError, then the loop exits.
...
* [BC] Changing the Guzzle\Http\Message\HttpResponse::setProtocol() method to accept a protocol and version in separate args.
